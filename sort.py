...
def insertionsort(arg,arglist):
    for indeks in range(len(arglist)):
        if ord(arg)<=ord(arglist[indeks]) or indeks==len(arglist)-1:
            return indeks
...
def mergesort(arglist):
    ...
    while l<L and r<R:#while len(arglistleft)>0 or len(arglistright)>0:
        if ord(arglistleft[0])<=ord(arglistright[0]):
            print("yes")
...
scrambledlist=[]
sortlist=[]
for i in range(0, 1000): #The scrambled list will be filled with 5 random letters
    scrambledlist.append(int(LETTERTONUMBER+random.randint(0,1000))) # This will append the converted random numbers to letters
...
else: #if user inputs 2 or anything else, the program will automatically perform merge sort
